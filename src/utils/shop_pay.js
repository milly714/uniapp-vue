...
const wePay = (data, successFun, failFun) => (
    uni.requestPayment({
        provider: payProvider,
        ...data,
        success(res) {
            successFun(JSON.stringify(res));
        },
        fail(err) {
            failFun(JSON.stringify(err))
        }
    })
);
// #ifdef H5
...
export default function weixinPay(params, successFun, failFun) {
    let fun = wePay;
